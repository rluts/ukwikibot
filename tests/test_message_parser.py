from wikibot.parser import MessageParser, Messages
...
async def test_get_response_link():
    ...
    assert message_type == Messages.LINK
...
async def test_get_image():
    ...
    response, message_type = await p.get_response()
    assert message_type == Messages.IMAGE
    assert len(response) == 1
    assert isinstance(response[0][0], bytes)
    assert response[0][1] == "Ronald Reagan"


async def test_get_coords():
    p = MessageParser("Де розташований Київ")
    response, message_type = await p.get_response()
    assert message_type == Messages.COORDS
    assert len(response) == 1
    lat, lon = response[0]
